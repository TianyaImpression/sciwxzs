...
translate_with_deepseek <- function(text, api_key, max_tokens = 10000) {
  ...
  tryCatch({
    ...
    if (status_code(response) == 200) {
      ...
      return(paste("失败：API状态码", status_code(response)))
    }
  }, error = function(e) {
    error_msg <- as.character(e$message)
    return(paste("失败：", error_msg))
  })
}
...
segment_chinese_with_deepseek <- function(text, api_key, doc_id = NULL, 
  ...
  tryCatch({
    ...
    if (status_code(response) == 200) {
      ...
      segmented_text <- gsub("\n", "", segmented_text)
      segmented_text <- gsub("\\s+", "", segmented_text)
              ...
              fluidRow(
                box(
                  ...
                  selectInput("wordcloud_shape", "形状:",
                              ...
                              selected = "圆形"),
                  colourInput("bg_color", "背景颜色:", value = "white"),
